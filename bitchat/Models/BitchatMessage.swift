...
final class BitchatMessage: Codable {
    ...
    var channel: String? // Local-only channel tag (e.g. "#site", "#defects") — not sent over BLE

    // Cached formatted text (not included in Codable)
    private var _cachedFormattedText: [String: AttributedString] = [:]
    
    func getCachedFormattedText(isDark: Bool, isSelf: Bool, bubble: Bool = false) -> AttributedString? {
        let key = bubble ? "bubble-\(isDark)-\(isSelf)" : "\(isDark)-\(isSelf)"
        return _cachedFormattedText[key]
    }

    func setCachedFormattedText(_ text: AttributedString, isDark: Bool, isSelf: Bool, bubble: Bool = false) {
        let key = bubble ? "bubble-\(isDark)-\(isSelf)" : "\(isDark)-\(isSelf)"
        _cachedFormattedText[key] = text
    }
    ...
    /// Create a copy of this message with different content (used for stripping channel tags)
    func withContent(_ newContent: String) -> BitchatMessage {
        let copy = BitchatMessage(
            id: id,
            sender: sender,
            content: newContent,
            timestamp: timestamp,
...
extension BitchatMessage {
    ...
    var relativeTimestamp: String {
        ...
        if interval < 60 {
            ...
            return Self.timeOnlyFormatter.string(from: timestamp)
        }
    }
}
