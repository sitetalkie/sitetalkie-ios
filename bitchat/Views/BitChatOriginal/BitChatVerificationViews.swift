...
struct BitChatMyQRView: View {
    ...
    var body: some View {
        VStack(spacing: 12) {
            ...
            VStack(spacing: 10) {
                BitChatQRCodeImage(data: qrString, size: 240)
...
struct BitChatQRCodeImage: View {
    let data: String
    let size: CGFloat
    ...
    var body: some View {
        Group {
            if let image = generateImage() {
                BitChatImageWrapper(image: image)
...
struct BitChatImageWrapper: View {
    let image: CGImage
    var body: some View {
        #if os(iOS)
...
struct BitChatQRScanView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            #if os(iOS)
            BitChatCameraScannerView(isActive: isActive) { code in
...
struct BitChatCameraScannerView: UIViewRepresentable {
...
struct BitChatVerificationSheetView: View {
    @EnvironmentObject var viewModel: ChatViewModel
                ...
                if showingScanner {
                    VStack(alignment: .leading, spacing: 12) {
                        Text("verification.scan.prompt_friend")
                            ...
                            .foregroundColor(accentColor)
                        #if os(iOS)
                        BitChatQRScanView(isActive: showingScanner, onSuccess: {
                            showingScanner = false
                        })
