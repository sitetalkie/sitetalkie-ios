...
struct BitChatContentView: View {
    ...
    @AppStorage("sitetalkie.bitchatMode") private var bitchatMode: Bool = true
...
// MARK: - Body
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            // Return to SiteTalkie banner
            Button(action: { showReturnAlert = true }) {
                HStack(spacing: 6) {
                    Spacer()
        ...
        #endif
        .alert("Return to SiteTalkie?", isPresented: $showReturnAlert) {
            Button("Stay on Mesh", role: .cancel) {}
            Button("Return") { bitchatMode = false }
        }
        ...
        .sheet(isPresented: Binding(
            ...
            if let peerID = viewModel.showingFingerprintFor {
                BitChatFingerprintView(viewModel: viewModel, peerID: peerID)
                    .environmentObject(viewModel)
            }
        }
#if os(iOS)
        ...
        .fullScreenCover(isPresented: Binding(
            ...
            BitChatImagePickerView(sourceType: imagePickerSourceType) { image in
                showImagePicker = false
                if let image = image {
                    Task {
                        do {
        ...
        .sheet(isPresented: Binding(
            ...
            BitChatMacImagePickerView { url in
                showMacImagePicker = false
                if let url = url {
                    Task {
                        do {
        ...
        .sheet(isPresented: Binding(
            ...
            if let url = imagePreviewURL {
                BitChatImagePreviewView(url: url)
                    .environmentObject(viewModel)
            }
        }
    ...
    private var mainHeaderView: some View {
        HStack(spacing: 0) {
            ...
            .sheet(isPresented: $showVerifySheet) {
                BitChatVerificationSheetView(isPresented: $showVerifySheet)
                    .environmentObject(viewModel)
            }
        }
        .frame(height: headerHeight)
        ...
        .sheet(isPresented: $showLocationChannelsSheet) {
            BitChatLocationChannelsSheet(isPresented: $showLocationChannelsSheet)
                .environmentObject(viewModel)
        ...
        .sheet(isPresented: $showLocationNotes, onDismiss: {
            ...
            Group {
                if let gh = notesGeohash ?? LocationChannelManager.shared.availableChannels.first(where: { $0.level == .building })?.geohash {
                    BitChatLocationNotesView(geohash: gh)
                        .environmentObject(viewModel)
                } else {
                    VStack(spacing: 12) {
                        HStack {
...
private extension BitChatContentView {
    ...
    private func messageRow(for message: BitchatMessage) -> some View {
        if message.sender == "system" {
            ...
            BitChatTextMessageView(message: message, expandedMessageIDs: $expandedMessageIDs)
        }
    }
            ...
            HStack(alignment: .center, spacing: 4) {
                ...
                if message.isPrivate && message.sender == viewModel.nickname,
                   let status = message.deliveryStatus {
                    BitChatDeliveryStatusView(status: status)
                        .padding(.leading, 4)
                }
            }
...
struct BitChatImagePreviewView: View {
    let url: URL
...
struct BitChatImagePickerView: UIViewControllerRepresentable {
...
struct BitChatMacImagePickerView: View {
