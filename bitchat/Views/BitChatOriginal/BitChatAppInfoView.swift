...
struct BitChatAppInfoView: View {
    ...
    private var infoContent: some View {
        VStack(alignment: .leading, spacing: 24) {
            ...
            VStack(alignment: .leading, spacing: 16) {
                BitChatSectionHeader(Strings.HowToUse.title)
            ...
            // Features
            VStack(alignment: .leading, spacing: 16) {
                BitChatSectionHeader(Strings.Features.title)

                BitChatFeatureRow(info: Strings.Features.offlineComm)

                BitChatFeatureRow(info: Strings.Features.encryption)

                BitChatFeatureRow(info: Strings.Features.extendedRange)

                BitChatFeatureRow(info: Strings.Features.favorites)

                BitChatFeatureRow(info: Strings.Features.geohash)

                BitChatFeatureRow(info: Strings.Features.mentions)
            }

            // Privacy
            VStack(alignment: .leading, spacing: 16) {
                BitChatSectionHeader(Strings.Privacy.title)

                BitChatFeatureRow(info: Strings.Privacy.noTracking)

                BitChatFeatureRow(info: Strings.Privacy.ephemeral)

                BitChatFeatureRow(info: Strings.Privacy.panic)
            }
        }
        .padding()
    }
...
struct BitChatAppInfoFeatureInfo {
    let icon: String
    let title: LocalizedStringKey
    let description: LocalizedStringKey
}

struct BitChatSectionHeader: View {
    let title: LocalizedStringKey
    @Environment(\.colorScheme) var colorScheme
...
struct BitChatFeatureRow: View {
    let info: BitChatAppInfoFeatureInfo
    @Environment(\.colorScheme) var colorScheme
