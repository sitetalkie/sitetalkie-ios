...
struct BitChatTextMessageView: View {
            ...
            if !lightningLinks.isEmpty || !cashuLinks.isEmpty {
                HStack(spacing: 8) {
                    ForEach(lightningLinks, id: \.self) { link in
                        BitChatPaymentChipView(paymentType: .lightning(link))
                    }
                    ForEach(cashuLinks, id: \.self) { link in
                        BitChatPaymentChipView(paymentType: .cashu(link))
                    }
                }
                .padding(.top, 6)
                .padding(.leading, 2)
...
#Preview {
    ...
    Group {
        List {
            BitChatTextMessageView(message: .preview, expandedMessageIDs: $ids)
