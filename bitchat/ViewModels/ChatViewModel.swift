...
final class ChatViewModel: ObservableObject, BitchatDelegate, CommandContextProvider, GeohashParticipantContext, MessageFormattingContext {
    ...
    /// Unread message counts per channel (keyed by ChannelType.rawValue, nil key = #site)
    @Published var channelUnreadCounts: [String: Int] = [:]
    /// Currently active UI channel (set by ContentView when user switches)
    @Published var activeUIChannel: String = "#site"
    ...
    let messagePersistence: MessagesPersistenceService
    ...
    init(
        ...
        identityManager: SecureIdentityStateManagerProtocol,
        transport: Transport,
        messagePersistence: MessagesPersistenceService
    ) {
        ...
        // Bulletin acknowledge — send BLE ack when user taps acknowledge
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(handleBulletinAcknowledge(_:)),
    ...
    @MainActor
    func sendMessage(_ content: String, channel: String? = nil) {
        ...
        // Handle pin broadcasts: send raw over mesh but show system message locally
        if trimmed.hasPrefix("[SITE_PIN:") {
            // Parse the pin for metadata
            var jsonStr = String(trimmed.dropFirst("[SITE_PIN:".count))
            if jsonStr.hasSuffix("]") { jsonStr = String(jsonStr.dropLast()) }
            if let data = jsonStr.data(using: .utf8),
               let pin = try? JSONDecoder().decode(SitePin.self, from: data) {
                addPinSystemMessage(createdBy: pin.createdBy, pinType: pin.type, title: pin.title, channels: pin.channels)
            }
            // Send raw content over mesh (always on #site)
            meshService.sendMessage(trimmed, mentions: [], messageID: UUID().uuidString, timestamp: Date())
            return
        }
        ...
        // Resolve channel type from raw string
        let channelType = ChannelType(rawValue: channel ?? "#site") ?? .site
        ...
        message.channel = (channelType == .site) ? nil : channelType.rawValue
        ...
        // Build wire content: prepend [CH:xxx] for non-site channels
        let wireContent = channelType.taggedContent(content)
        let wireTrimmed = channelType.taggedContent(trimmed)

        updateChannelActivityTimeThenSend(content: wireContent,
                                          trimmed: wireTrimmed,
                                          mentions: mentions,
    ...
    func startPrivateChat(with peerID: PeerID) {
        ...
        if privateChats[peerID] == nil || privateChats[peerID]?.isEmpty == true {
            let conversationKey = "dm:\(peerID.id)"
            let persistedMessages = messagePersistence.loadMessages(for: conversationKey, limit: 100)
            if !persistedMessages.isEmpty {
                privateChats[peerID] = persistedMessages
                SecureLogger.info("💾 Loaded \(persistedMessages.count) persisted messages for DM with \(peerNickname)", category: .session)
            }
        }
    ...
    @MainActor
    @objc private func handleBulletinAcknowledge(_ notification: Notification) {
        guard let bulletinId = notification.userInfo?["bulletinId"] as? Int else { return }
        let ackMessage = "[BULLETIN_ACK:\(bulletinId)]"
        meshService.sendMessage(ackMessage, mentions: [], messageID: UUID().uuidString, timestamp: Date())
    }
    ...
    func panicClearAllData() {
        ...
        // Clear persisted messages from keychain
    ...
    // MARK: - Bubble Formatting (Construction Design System)

    /// Formats message content only (no header, no timestamp) for bubble display.
    /// Uses dark text for self bubbles (amber bg) and light text for others' bubbles (dark bg).
    @MainActor
    func formatBubbleContent(_ message: BitchatMessage, colorScheme: ColorScheme) -> AttributedString {
        let isSelf = isSelfMessage(message)
        let isDark = colorScheme == .dark

        // Check bubble-specific cache
        if let cached = message.getCachedFormattedText(isDark: isDark, isSelf: isSelf, bubble: true) {
            return cached
        }

        // Bubble text colors: dark on amber (self), light on dark card (others)
        let baseColor: Color = isSelf
        ...
        if (content.count > 4000 || content.hasVeryLongToken(threshold: 1024)) && !containsCashuEarly {
            ...
            plainStyle.foregroundColor = baseColor
            plainStyle.font = isSelf
                ? .bitchatSystem(size: contentFontSize, weight: .semibold)
                : .bitchatSystem(size: contentFontSize)
            result.append(AttributedString(content).mergingAttributes(plainStyle))
        } else {
            let hashtagRegex = Patterns.hashtag
            let mentionRegex = Patterns.mention
            ...
            let occupied: [NSRange] = urlMatches.map { $0.range } + lightningMatches.map { $0.range(at: 0) }
            func overlapsOccupied(_ r: NSRange) -> Bool {
                for or2 in occupied { if NSIntersectionRange(r, or2).length > 0 { return true } }
                return false
            }
            for match in bolt11Matches where !overlapsMention(match.range(at: 0)) && !overlapsOccupied(match.range(at: 0)) {
                allMatches.append((match.range(at: 0), "bolt11"))
            ...
            for (range, type) in allMatches {
                if let nsRange = Range(range, in: content) {
                    ...
                    if type == "mention" {
                        let (mBase, mSuffix) = matchText.splitSuffix()
                        var mentionStyle = AttributeContainer()
                        mentionStyle.font = .bitchatSystem(size: contentFontSize, weight: .semibold)
                        // Use amber highlight for mentions on dark bubble, keep base on amber bubble
                        mentionStyle.foregroundColor = isSelf ? baseColor : Color(red: 0.910, green: 0.588, blue: 0.047)
                        let at = "@"
                        result.append(AttributedString("\(at)").mergingAttributes(mentionStyle))
                        result.append(AttributedString(mBase).mergingAttributes(mentionStyle))
                        if !mSuffix.isEmpty {
                            var light = mentionStyle
                            light.foregroundColor = mentionStyle.foregroundColor?.opacity(0.6) ?? baseColor.opacity(0.6)
                            result.append(AttributedString(mSuffix).mergingAttributes(light))
                        }
                    } else if type == "cashu" || type == "lightning" || type == "bolt11" || type == "lnurl" {
                        var spacer = AttributeContainer()
                        spacer.foregroundColor = baseColor
                        spacer.font = .bitchatSystem(size: contentFontSize)
                        result.append(AttributedString(" ").mergingAttributes(spacer))
                    } else if type == "url" {
                        var urlStyle = AttributeContainer()
                        urlStyle.font = .bitchatSystem(size: contentFontSize, weight: .medium)
                        urlStyle.foregroundColor = isSelf ? Color(red: 0.102, green: 0.110, blue: 0.125) : Color(red: 0.4, green: 0.7, blue: 1.0)
                        urlStyle.underlineStyle = .single
                        if let url = URL(string: matchText) { urlStyle.link = url }
                        result.append(AttributedString(matchText).mergingAttributes(urlStyle))
                    } else if type == "hashtag" {
                        let token = String(matchText.dropFirst()).lowercased()
                        let allowed = Set("0123456789bcdefghjkmnpqrstuvwxyz")
                        let isGeohash = (2...12).contains(token.count) && token.allSatisfy { allowed.contains($0) }
                        var tagStyle = AttributeContainer()
                        tagStyle.font = .bitchatSystem(size: contentFontSize)
                        tagStyle.foregroundColor = baseColor
                        if isGeohash, let url = URL(string: "sitetalkie://geohash/\(token)") {
                            tagStyle.link = url
                            tagStyle.underlineStyle = .single
                        }
                        result.append(AttributedString(matchText).mergingAttributes(tagStyle))
        ...
        message.setCachedFormattedText(result, isDark: isDark, isSelf: isSelf, bubble: true)
        return result
    }
    ...
    // MARK: - Channel Unread Tracking

    /// Mark a channel as read (clear unread count)
    @MainActor
    func markChannelAsRead(_ channelRaw: String) {
        activeUIChannel = channelRaw
        channelUnreadCounts[channelRaw] = 0
    }

    /// Increment unread count for a channel if it's not the active UI channel
    @MainActor
    private func incrementUnreadIfNeeded(for channelRaw: String) {
        guard channelRaw != activeUIChannel else { return }
        channelUnreadCounts[channelRaw, default: 0] += 1
    }
    ...
    private func loadPersistedMessages() {
        // Load public mesh messages
        let publicKey = "public:#site"
        let publicMessages = messagePersistence.loadMessages(for: publicKey, limit: 100)
        self.messages = publicMessages

        SecureLogger.info("💾 Loaded \(publicMessages.count) persisted messages for #site channel", category: .session)

        // Private messages loaded lazily when conversation opens
    }
    ...
    func didReceivePublicMessage(from peerID: PeerID, nickname: String, content: String, timestamp: Date, messageID: String?) {
        Task { @MainActor in
            ...
            // Handle notifications/haptics based on app state
            handlePublicChannelNotification(for: msg)
    ...
    func didDisconnectFromPeer(_ peerID: PeerID) {
        ...
        if let current = selectedPrivateChatPeer, current == peerID, let stableKeyHex = derivedStableKeyHex {
            ...
            if let messages = privateChats[peerID] {
                ...
                guard let chat = privateChats[stableKeyHex] else { return }
                let existing = Set(chat.map { $0.id })
    ...
    func handlePublicMessage(_ message: BitchatMessage) {
        var finalMessage = processActionMessage(message)

        // Handle incoming site pins — intercept before regular pipeline
        if finalMessage.content.hasPrefix("[SITE_PIN:") {
            handleIncomingSitePin(finalMessage.content)
            return
        }

        // Handle pin/snag resolved messages from mesh
        if finalMessage.content.hasPrefix("[SITE_PIN_RESOLVED:") {
            handleIncomingPinResolved(finalMessage.content)
            return
        }

        // Handle pin/snag deleted messages from mesh
        if finalMessage.content.hasPrefix("[SITE_PIN_DELETED:") {
            handleIncomingPinDeleted(finalMessage.content)
            return
        }

        // Handle snag status updates from mesh
        if finalMessage.content.hasPrefix("[SNAG_PROGRESS:") {
            handleIncomingSnagProgress(finalMessage.content)
            return
        }

        // Handle incoming bulletin messages — intercept before regular pipeline
        if finalMessage.content.hasPrefix("[BULLETIN:") {
            handleIncomingBulletin(finalMessage)
            return
        }

        // Handle incoming bulletin acks — ignore (firmware-level only)
        if finalMessage.content.hasPrefix("[BULLETIN_ACK:") {
            return
        }

        // Parse [CH:xxx] channel tag from message content
        // Site alerts always go to #site regardless of any tag
        let isSiteAlert = finalMessage.content.hasPrefix("[SITE_ALERT:")
        if !isSiteAlert && finalMessage.content.hasPrefix("[CH:") {
            let (parsedChannel, strippedContent) = ChannelType.parseChannelTag(from: finalMessage.content)
            finalMessage = finalMessage.withContent(strippedContent)
            finalMessage.channel = parsedChannel.rawValue
        }
        // Messages without a [CH:] tag (including site alerts) default to #site (channel = nil)

        // Ingest [SNAG:...] messages into SnagStore (but don't return — let them show in chat)
        if finalMessage.content.hasPrefix("[SNAG:") {
            if let parsed = SnagMessage.parse(from: finalMessage.content) {
                let (baseName, _) = finalMessage.sender.splitSuffix()
                SnagStore.shared.addFromMesh(parsed, sender: baseName)
            }
        }
        ...
        if !isGeo && finalMessage.sender != "system" {
            ...
            messagePersistence.appendMessage(finalMessage, to: "public:#site")
        }
        ...
        if isGeo && finalMessage.sender != "system" {
            if let gh = currentGeohash {
                ...
                // Persist to long-term storage
                messagePersistence.appendMessage(finalMessage, to: "public:#geo:\(gh)")
            }
        }
        ...
        if !isSystem {
            let msgChannel = finalMessage.channel ?? "#site"
            incrementUnreadIfNeeded(for: msgChannel)
        }
    ...
    // MARK: - Site Pin Mesh Receive

    /// Parse and save a site pin received from the mesh.
    @MainActor
    private func handleIncomingSitePin(_ content: String) {
        // Format: [SITE_PIN:{json}] or [SITE_PIN:{json}
        var jsonString = content
        if jsonString.hasPrefix("[SITE_PIN:") {
            jsonString = String(jsonString.dropFirst("[SITE_PIN:".count))
        }
        ...
        guard let data = jsonString.data(using: .utf8),
              let pin = try? JSONDecoder().decode(SitePin.self, from: data) else {
            return
        }

        // Add to local storage (addPin checks for duplicate IDs)
        SitePinManager.shared.addPin(pin)

        // Add system message with channel routing
        addPinSystemMessage(createdBy: pin.createdBy, pinType: pin.type, title: pin.title, channels: pin.channels)

        // Post notification for UI toast
        NotificationCenter.default.post(
            name: .sitePinReceived,
            object: nil,
            userInfo: ["createdBy": pin.createdBy]
        )
    }

    /// Handle incoming bulletin from mesh: [BULLETIN:{id}:{ACK|INFO}] {title}\n{content}
    @MainActor
    private func handleIncomingBulletin(_ message: BitchatMessage) {
        guard let parsed = BulletinMessage.parse(from: message.content) else { return }

        // Add to bulletin store
        BulletinStore.shared.addFromMesh(parsed)

        // Insert a system notification card in #site channel
        let noticeContent = "[BULLETIN_NOTICE:\(parsed.id):\(parsed.title)]"
        let systemMessage = BitchatMessage(
            sender: "system",
        ...
        systemMessage.channel = nil // #site
        messages.append(systemMessage)

        // Post notification for UI (badge update, toast)
        NotificationCenter.default.post(
            name: .bulletinReceived,
            object: nil,
            userInfo: [
                "bulletinId": parsed.id,
                "title": parsed.title,
        ...
        var body = parsed.title
        if parsed.requiresAck {
            body += " — Acknowledgment required"
        }
        NotificationService.shared.sendLocalNotification(
            title: "Site Bulletin",
            body: body,
            identifier: "bulletin-\(parsed.id)-\(Date().timeIntervalSince1970)",
            userInfo: ["bulletinId": parsed.id],
            interruptionLevel: .active
        )
    }

    /// Handle incoming pin resolved message from mesh: [SITE_PIN_RESOLVED:pinId]
    @MainActor
    private func handleIncomingPinResolved(_ content: String) {
        var pinId = content
        if pinId.hasPrefix("[SITE_PIN_RESOLVED:") {
            pinId = String(pinId.dropFirst("[SITE_PIN_RESOLVED:".count))
        }
        if pinId.hasSuffix("]") {
            pinId = String(pinId.dropLast())
        }
        // Check pins first
        if let pin = SitePinManager.shared.pins.first(where: { $0.id == pinId }) {
            SitePinManager.shared.resolvePin(pin)
            addPinResolvedMessage(resolvedBy: pin.createdBy, title: pin.title, channels: pin.channels)
        }
        // Also check snags
        SnagStore.shared.updateStatus(pinId, status: .resolved)
    }

    /// Handle incoming pin/snag deleted message from mesh: [SITE_PIN_DELETED:id]
    @MainActor
    private func handleIncomingPinDeleted(_ content: String) {
        var pinId = content
        if pinId.hasPrefix("[SITE_PIN_DELETED:") {
            pinId = String(pinId.dropFirst("[SITE_PIN_DELETED:".count))
        }
        if pinId.hasSuffix("]") {
            pinId = String(pinId.dropLast())
        }
        // Check pins first
        if let pin = SitePinManager.shared.pins.first(where: { $0.id == pinId }) {
            addPinDeletedMessage(deletedBy: pin.createdBy, title: pin.title, channels: pin.channels)
            SitePinManager.shared.removePin(pin)
        }
        // Also check snags
        SnagStore.shared.delete(pinId)
    }

    /// Handle incoming snag progress message from mesh: [SNAG_PROGRESS:snagId]
    @MainActor
    private func handleIncomingSnagProgress(_ content: String) {
        var snagId = content
        if snagId.hasPrefix("[SNAG_PROGRESS:") {
            snagId = String(snagId.dropFirst("[SNAG_PROGRESS:".count))
        }
        if snagId.hasSuffix("]") {
            snagId = String(snagId.dropLast())
        }
        SnagStore.shared.updateStatus(snagId, status: .inProgress)
    }

    /// Add pin creation system messages to chat — one per channel.
    /// Format: [PIN_NOTICE:createdBy:pinTypeRaw:title:channelsCSV]
    @MainActor
    func addPinSystemMessage(createdBy: String, pinType: PinType, title: String = "", channels: [String] = ["all"]) {
        let channelsCSV = channels.joined(separator: ",")
        let content = "[PIN_NOTICE:\(createdBy):\(pinType.rawValue):\(title):\(channelsCSV)]"
        appendPinSystemMessages(content: content, channels: channels)
    }

    /// Add pin resolved system messages — one per channel.
    /// Format: [PIN_RESOLVED:resolverName:title]
    @MainActor
    func addPinResolvedMessage(resolvedBy: String, title: String, channels: [String] = ["all"]) {
        let content = "[PIN_RESOLVED:\(resolvedBy):\(title)]"
        appendPinSystemMessages(content: content, channels: channels)
    }

    /// Add pin deleted system messages — one per channel.
    /// Format: [PIN_DELETED:deleterName:title]
    @MainActor
    func addPinDeletedMessage(deletedBy: String, title: String, channels: [String] = ["all"]) {
        let content = "[PIN_DELETED:\(deletedBy):\(title)]"
        appendPinSystemMessages(content: content, channels: channels)
    }

    /// Add a pin expired system message (shows on all channels).
    /// Format: [PIN_EXPIRED:title]
    @MainActor
    func addPinExpiredMessage(title: String) {
        let content = "[PIN_EXPIRED:\(title)]"
        let systemMessage = BitchatMessage(
            sender: "system",
            content: content,
            timestamp: Date(),
        ...
        systemMessage.channel = nil
        messages.append(systemMessage)
    }

    /// Helper: create one system message per channel, or one global message if channels contains "all".
    @MainActor
    private func appendPinSystemMessages(content: String, channels: [String]) {
        if channels.contains("all") {
            // Single message visible on all channels
            let msg = BitchatMessage(sender: "system", content: content, timestamp: Date(), isRelay: false)
            msg.channel = nil
            messages.append(msg)
        } else {
            // One message per selected channel
            for ch in channels {
                let msg = BitchatMessage(sender: "system", content: content, timestamp: Date(), isRelay: false)
                msg.channel = "#\(ch)"
                messages.append(msg)
    ...
    /// Resolve a pin: update locally, broadcast over mesh, add system message.
    @MainActor
    func resolveAndBroadcastPin(_ pin: SitePin) {
        SitePinManager.shared.resolvePin(pin)
        addPinResolvedMessage(resolvedBy: nickname, title: pin.title, channels: pin.channels)
        let message = "[SITE_PIN_RESOLVED:\(pin.id)]"
        meshService.sendMessage(message, mentions: [], messageID: UUID().uuidString, timestamp: Date())
    }

    /// Delete a pin: remove locally, broadcast over mesh, add system message.
    @MainActor
    func deleteAndBroadcastPin(_ pin: SitePin) {
        SitePinManager.shared.removePin(pin)
        addPinDeletedMessage(deletedBy: nickname, title: pin.title, channels: pin.channels)
        let message = "[SITE_PIN_DELETED:\(pin.id)]"
        meshService.sendMessage(message, mentions: [], messageID: UUID().uuidString, timestamp: Date())
    }

    /// Broadcast snag "in progress" status update over mesh.
    @MainActor
    func broadcastSnagProgress(_ snagId: String) {
        SnagStore.shared.updateStatus(snagId, status: .inProgress)
        let message = "[SNAG_PROGRESS:\(snagId)]"
        meshService.sendMessage(message, mentions: [], messageID: UUID().uuidString, timestamp: Date())
    }

    /// Resolve a snag: update store, broadcast over mesh.
    @MainActor
    func resolveAndBroadcastSnag(_ snagId: String) {
        SnagStore.shared.updateStatus(snagId, status: .resolved)
        let message = "[SITE_PIN_RESOLVED:\(snagId)]"
        meshService.sendMessage(message, mentions: [], messageID: UUID().uuidString, timestamp: Date())
    }

    /// Delete a snag: remove from store, broadcast over mesh.
    @MainActor
    func deleteAndBroadcastSnag(_ snagId: String) {
        SnagStore.shared.delete(snagId)
        let message = "[SITE_PIN_DELETED:\(snagId)]"
        meshService.sendMessage(message, mentions: [], messageID: UUID().uuidString, timestamp: Date())
    }

    /// Broadcast a snag photo over BLE mesh as a file transfer.
    @MainActor
    func sendSnagPhoto(jpegData: Data, fileName: String) {
        guard jpegData.count <= FileTransferLimits.maxImageBytes else {
            SecureLogger.warning("Snag photo exceeds size limit (\(jpegData.count) bytes)", category: .session)
            return
        }
        ...
        guard packet.encode() != nil else {
            SecureLogger.error("Failed to encode snag photo packet", category: .session)
            return
        }
        let message = enqueueMediaMessage(content: "[image] \(fileName)", targetPeer: nil)
    ...
    /// Handle public channel notifications based on app state
    @MainActor
    private func handlePublicChannelNotification(for message: BitchatMessage) {
        // Don't notify for own messages or system messages
        guard message.sender != nickname && message.sender != "system" else { return }

        // Check if enabled in settings
        guard UserDefaults.standard.bool(forKey: "com.sitetalkie.notifications.publicChannels") else {
            return
        }
        ...
        if message.senderPeerID?.isGeoChat == true {
            // Location channel
            guard let geo = currentGeohash else { return }
            channelName = "#\(geo)"
            channelID = "geo:\(geo)"

            if case .location(let ch) = activeChannel, ch.geohash == geo {
        ...
        #if os(iOS)
        if UIApplication.shared.applicationState == .active && isViewingChannel {
            // Foreground + viewing channel: subtle haptic only
            playSubtleHaptic()
        } else if UIApplication.shared.applicationState != .active {
            // Background: send notification
            let displayMessage = message.content.count > 100
                ? String(message.content.prefix(100)) + "..."
                : message.content

            NotificationService.shared.sendPublicChannelNotification(
    ...
    /// Play subtle haptic for foreground public messages
