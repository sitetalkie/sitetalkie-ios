...
extension ChatViewModel {
    ...
    func sendPrivateMessage(_ content: String, to peerID: PeerID) {
        ...
        messagePersistence.appendMessage(message, to: conversationKey)
    ...
    func handlePrivateMessage(
        ...
        // Persist to long-term storage
        let conversationKey = "dm:\(convKey.id)"
        messagePersistence.appendMessage(msg, to: conversationKey)
    ...
    @MainActor
    func sendImage(from sourceURL: URL, targetPeer: PeerID? = nil, cleanup: (() -> Void)? = nil) {
        guard canSendMediaInCurrentContext else {
        ...
        // Use the provided targetPeer if specified, otherwise fall back to selectedPrivateChatPeer
        let targetPeer = targetPeer ?? selectedPrivateChatPeer
    ...
    @MainActor
    func sendDocument(from sourceURL: URL, fileName: String, targetPeer: PeerID? = nil) {
        guard canSendMediaInCurrentContext else {
            SecureLogger.info("Document send blocked outside mesh/private context", category: .session)
            try? FileManager.default.removeItem(at: sourceURL)
            addSystemMessage("Documents are only available in mesh chats.")
            return
        }

        let targetPeer = targetPeer ?? selectedPrivateChatPeer
        let message = enqueueMediaMessage(content: "[file] \(fileName)", targetPeer: targetPeer)
        let messageID = message.id
        let transferId = makeTransferID(messageID: messageID)

        // Resolve outgoing directory on main actor before entering detached task
        let filesBase: URL?
        do {
            filesBase = try applicationFilesDirectory()
        } catch {
            SecureLogger.error("Failed to resolve files directory: \(error)", category: .session)
            filesBase = nil
        }
        ...
        Task.detached(priority: .userInitiated) { [weak self] in
            ...
            do {
                ...
                guard let fileSize = attrs[.size] as? Int,
                      fileSize <= FileTransferLimits.maxPayloadBytes else {
                    let size = (attrs[.size] as? Int) ?? 0
                    SecureLogger.warning("Document exceeds size limit (\(size) bytes)", category: .session)
                    try? FileManager.default.removeItem(at: sourceURL)
                    await MainActor.run {
                        self.handleMediaSendFailure(messageID: messageID, reason: "Document too large")
                    }
                    return
                }
                ...
                switch ext {
                case "pdf": mimeString = "application/pdf"
                default:    mimeString = "application/octet-stream"
                }

                // Copy to outgoing directory for local display
                if let base = filesBase {
                    let outDir = base.appendingPathComponent("files/outgoing", isDirectory: true)
                    try FileManager.default.createDirectory(at: outDir, withIntermediateDirectories: true)
                    let destURL = outDir.appendingPathComponent(fileName)
                ...
                // Clean up the temp copy from document picker
                try? FileManager.default.removeItem(at: sourceURL)
            } catch {
                SecureLogger.error("Document send failed: \(error)", category: .session)
                await MainActor.run {
                    self.handleMediaSendFailure(messageID: messageID, reason: "Failed to send document")
                }
            }
        }
    }
    ...
    func addMessageToPrivateChatsIfNeeded(_ message: BitchatMessage, targetPeerID: PeerID) {
        ...
        // Persist to long-term storage
        let conversationKey = "dm:\(targetPeerID.id)"
        messagePersistence.appendMessage(message, to: conversationKey)
