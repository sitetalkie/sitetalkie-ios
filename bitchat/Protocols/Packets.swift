...
struct AnnouncementPacket {
    ...
    let trade: String?                  // Construction trade/role
    let latitude: Double?               // GPS latitude (WGS84)
    let longitude: Double?              // GPS longitude (WGS84)
    let altitude: Double?               // GPS altitude (meters above sea level)
    ...
    func encode() -> Data? {
        ...
        // TLV for trade (optional)
        if let trade = trade, !trade.isEmpty {
            guard let tradeData = trade.data(using: .utf8), tradeData.count <= 255 else { return nil }
            data.append(TLVType.trade.rawValue)
            data.append(UInt8(tradeData.count))
            data.append(tradeData)
        }

        // TLV for GPS location (optional, 8 bytes each as IEEE 754 Double)
        if let lat = latitude {
            var value = lat
            let bytes = withUnsafeBytes(of: &value) { Data($0) }
            data.append(TLVType.latitude.rawValue)
        ...
        if let lon = longitude {
            var value = lon
            let bytes = withUnsafeBytes(of: &value) { Data($0) }
            data.append(TLVType.longitude.rawValue)
            data.append(UInt8(bytes.count))
            data.append(bytes)
        }
        if let alt = altitude {
            var value = alt
            let bytes = withUnsafeBytes(of: &value) { Data($0) }
            data.append(TLVType.altitude.rawValue)
            data.append(UInt8(bytes.count))
            data.append(bytes)
        }
    ...
    static func decode(from data: Data) -> AnnouncementPacket? {
        ...
        while offset + 2 <= data.count {
            ...
            if let type = TLVType(rawValue: typeRaw) {
                ...
                case .trade:
                    trade = String(data: value, encoding: .utf8)
                case .latitude:
                    if length == MemoryLayout<Double>.size {
                        latitude = Data(value).withUnsafeBytes { $0.load(as: Double.self) }
                    }
                case .longitude:
                    if length == MemoryLayout<Double>.size {
                        longitude = Data(value).withUnsafeBytes { $0.load(as: Double.self) }
                    }
                case .altitude:
                    if length == MemoryLayout<Double>.size {
                        altitude = Data(value).withUnsafeBytes { $0.load(as: Double.self) }
                    }
                }
            } else {
