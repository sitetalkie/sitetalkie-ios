...
final class BLEService: NSObject {
    ...
    private static let centralRestorationID = "com.sitetalkie.app.ble.central"
    private static let peripheralRestorationID = "com.sitetalkie.app.ble.peripheral"
    ...
    /// Max age before a peer's last-known-good GPS coordinates are treated as stale.
    private static let peerGPSStaleness: TimeInterval = 300 // 5 minutes
    private var peers: [PeerID: PeerInfo] = [:]
    private var peripheralLastRSSI: [String: Int] = [:]  // Peripheral UUID -> last discovery RSSI
    ...
    init(
        ...
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(handleFirstGPSFix),
            name: RadarLocationManager.firstReliableGPSFixNotification,
            object: nil
        )
        #endif
    ...
    private func handleFileTransfer(_ packet: BitchatPacket, from peerID: PeerID) {
        ...
        // Snag photos are attached to site pins, not standalone chat messages.
        // They transfer over BLE for the SiteNode dashboard but should not appear in chat.
        if (filePacket.fileName ?? "").hasPrefix("snag_") {
            SecureLogger.debug("📁 Snag photo received — suppressed from chat UI", category: .session)
            return
        }
    ...
    private func sendAnnounce(forceSend: Bool = false) {
        ...
        // Get trade from UserDefaults
        let userTrade = UserDefaults.standard.string(forKey: "com.sitetalkie.user.trade")

        // Get current GPS location for radar positioning (thread-safe reads)
        // Ghost Mode: suppress location data so we don't appear on others' radars with position
        let ghostMode = UserDefaults.standard.bool(forKey: "ghostModeEnabled")
        let radarLoc = RadarLocationManager.shared
        let lat: Double? = (!ghostMode && radarLoc.latestHasBroadcastableGPS) ? radarLoc.latestLatitude : nil
        let lon: Double? = (!ghostMode && radarLoc.latestHasBroadcastableGPS) ? radarLoc.latestLongitude : nil
        let alt: Double? = (!ghostMode && radarLoc.latestHasBroadcastableGPS) ? radarLoc.latestAltitude : nil
...
extension BLEService: CBCentralManagerDelegate {
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral, advertisementData: [String: Any], rssi RSSI: NSNumber) {
        ...
        peripheralLastRSSI[peripheralID] = rssiValue
...
func centralManager(_ central: CBCentralManager, didConnect peripheral: CBPeripheral) {
    ...
    func centralManager(_ central: CBCentralManager, didDisconnectPeripheral peripheral: CBPeripheral, error: Error?) {
        ...
        peripheralLastRSSI.removeValue(forKey: peripheralID)
...
extension BLEService: CBPeripheralManagerDelegate {
    ...
    func peripheralManager(_ peripheral: CBPeripheralManager, didReceiveRead request: CBATTRequest) {
        // Always respond with .success — never return auth/encryption errors
        // which would trigger iOS pairing dialogs. App-layer Noise XX handles security.
...
extension BLEService {
    ...
    /// Re-announce with GPS data once the first reliable fix arrives this session.
    @objc private func handleFirstGPSFix() {
        messageQueue.async { [weak self] in
            self?.sendAnnounce(forceSend: true)
        }
    }
    ...
    private func handleAnnounce(_ packet: BitchatPacket, from peerID: PeerID) {
        ...
        // Look up RSSI for this peer via peripheral mapping
        let peerRSSI: Int? = {
            if let uuid = peerToPeripheralUUID[peerID] {
                return peripheralLastRSSI[uuid]
            }
            return nil
        }()
        ...
        collectionsQueue.sync(flags: .barrier) {
            ...
            // Update or create peer info, preserving last-known-good GPS
            let now = Date()
            let resolvedLat: Double?
            let resolvedLon: Double?
            let resolvedAlt: Double?
            let resolvedGPSTime: Date?

            if let lat = announcement.latitude, let lon = announcement.longitude {
                // New announce has GPS → use it
                resolvedLat = lat
                resolvedLon = lon
                resolvedAlt = announcement.altitude
                resolvedGPSTime = now
            } else if let existing = existingPeer,
                      let prevLat = existing.latitude, let prevLon = existing.longitude,
                      let lastGPS = existing.lastGPSUpdate,
                      now.timeIntervalSince(lastGPS) <= Self.peerGPSStaleness {
                // No GPS in announce but previous values are fresh → keep them
                resolvedLat = prevLat
                resolvedLon = prevLon
                resolvedAlt = existing.altitude
                resolvedGPSTime = lastGPS
            } else {
                // No GPS available or stale → nil
                resolvedLat = nil
                resolvedLon = nil
                resolvedAlt = nil
                resolvedGPSTime = nil
            ...
            if let existing = existingPeer, existing.isConnected {
                ...
                peers[peerID] = PeerInfo(
                    ...
                    isVerifiedNickname: true,
                    lastSeen: now,
                    rssi: peerRSSI ?? existing.rssi,
                    latitude: resolvedLat,
                    longitude: resolvedLon,
                    altitude: resolvedAlt,
                    lastGPSUpdate: resolvedGPSTime
                )
            } else {
